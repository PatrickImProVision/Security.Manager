...
class User extends BaseController
{
    ...
    public function create(): ResponseInterface
    {
        ...
        $security->backfillCangColumn(SecurityCangService::USER_URL_ID, 'users');
        ...
        $deactivationGuid = $this->newGuid('deactivation_guid');
    ...
    public function createUser(): ResponseInterface
    {
        ...
        try {
            $db->table('users')->insert([
                ...
                'profile_image'     => null,
                'is_active'         => $isActive,
                'activation_guid'   => $isActive ? null : $this->newGuid('activation_guid'),
                'deactivation_guid' => $this->newGuid('deactivation_guid'),
                'reset_guid'        => null,
                'last_login_at'     => null,
                'created_at'        => date('Y-m-d H:i:s'),
                'updated_at'        => null,
    ...
    public function update(int $id): ResponseInterface
    {
        ...
        if ($this->canChangeUserRole($current, $user)) {
            ...
            $data['activation_guid'] = $data['is_active']
                ? null
                : ((string) ($user['activation_guid'] ?? '') !== '' ? (string) $user['activation_guid'] : $this->newGuid('activation_guid'));
    ...
    private function newGuid(string $column = 'activation_guid'): string
    {
        try {
            $key = (new SecurityCangService())->generateFor(SecurityCangService::PASSWORD_ID, 'users', $column);
            if ($key !== null) {
                return $key;
            }
        } catch (\Throwable) {
            // Fall back to the shipped random token behavior if CANG is unavailable.
        }
